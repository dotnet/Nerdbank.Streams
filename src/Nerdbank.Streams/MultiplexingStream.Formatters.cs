...
namespace Nerdbank.Streams
{
    ...
    public partial class MultiplexingStream
    {
        ...
        {
            private const sbyte PayloadExtensionTypeCode = 1;
            ...
            internal override void WriteFrame(FrameHeader header, ReadOnlySequence<byte> payload)
            {
                ...
                if (elementCount > 1)
                {
                    ...
                    if (elementCount > 2)
                    {
                        writer.WriteExtensionFormat(new ExtensionResult(PayloadExtensionTypeCode, payload));
                    }
                }
            ...
            private static (FrameHeader Header, ReadOnlySequence<byte> Payload) DeserializeFrame(ReadOnlySequence<byte> frameSequence)
            {
                ...
                if (headerElementCount > 1)
                {
                    ...
                    {
                        var extensionResult = reader.ReadExtensionFormat();
                        if (extensionResult.TypeCode != PayloadExtensionTypeCode)
                        {
                            throw new MultiplexingProtocolException("Unexpected type code for payload.");
                        }

                        return (header, extensionResult.Data);
                    }
                }
