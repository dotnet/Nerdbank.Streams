...
;[1, 2, 3].forEach(protocolMajorVersion => {
	describe(`MultiplexingStream v${protocolMajorVersion} (interop) `, () => {
		...
		const entrypointDllPath = `${__dirname}/../../../../bin/Nerdbank.Streams.Interop.Tests/Debug/net8.0/Nerdbank.Streams.Interop.Tests.dll`
		...
		beforeEach(async () => {
			proc = spawn('dotnet', [entrypointDllPath, protocolMajorVersion.toString()], spawnOptions.env as any)
			try {
				procExited = new Deferred<any>()
				proc.once('error', err => procExited.resolve(err))
				proc.once('exit', code => procExited.resolve(code))
		...
		it('Can offer error completed channel', async () => {
			...
			channelToCompleteWithError.completion
				.then(_ => {
		...
		it('Can accept error completed channel', async () => {
			...
			channelCompletedWithError.completion
				.then(async _ => {
		...
		function writeAsync(stream: NodeJS.WritableStream, text: string): Promise<void> {
			...
			stream.write(text, 'utf8', (err: Error | null | undefined) => {
				if (err) {
					deferred.reject(err)
				} else {
					deferred.resolve()
