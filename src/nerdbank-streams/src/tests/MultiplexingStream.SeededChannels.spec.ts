...
;[3].forEach(protocolMajorVersion => {
	describe(`MultiplexingStream v${protocolMajorVersion} seeded channels`, () => {
		...
		it('can send content', async () => {
			const MX1_CH0 = mx1.acceptChannel(0)
			const MX1_CH1 = mx1.acceptChannel(1)
			const MX2_CH0 = mx2.acceptChannel(0)
			const MX2_CH1 = mx2.acceptChannel(1)

			MX1_CH0.stream.write('abc')
			expect(await getBufferFrom(MX2_CH0.stream, 3)).toEqual(Buffer.from('abc'))

			MX2_CH1.stream.write('abc')
			expect(await getBufferFrom(MX1_CH1.stream, 3)).toEqual(Buffer.from('abc'))
		})
